...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    ...
    async_add_entities([
        PanResetButton(hass),
        PanSaveButton(hass),
        PanDeleteButton(hass),
        PanToggleYeastButton(hass)
    ], True)
...
class PanToggleYeastButton(ButtonEntity):
    """Boton nativo para alternar entre levadura fresca y seca con conmutacion real."""
    ...
    async def async_press(self) -> None:
        """Alterna el tipo de levadura en memoria y escala dividiendo o multiplicando."""
        global _TIPO_LEVADURA_MEMORIA
        ...
        if _TIPO_LEVADURA_MEMORIA == "seca":
