...
class PanSelectMenu(SelectEntity):
    """Representacion nativa de un menu desplegable panadero."""
    ...
    def options(self) -> list:
        """Filtra dinamicamente las harinas en vivo y fuerza la seleccion de respaldo."""
        if self._clave == "harina_para_prefermento":
            try:
                ...
                peso_h_total = get_f("sensor.pan_harina_total")
                peso_pref = get_f("sensor.pan_prefermento_total")
    ...
    async def async_added_to_hass(self):
        """Se ejecuta cuando el desplegable se acopla a HA. Lee el JSON de forma segura."""
        ...
        if self._clave in ["harina_para_prefermento", "tipo_de_prefermento"]:
            @callback
            def _on_bascula_change(event):
                if self._current_option == "unknown" or self._current_option not in self.options:
                    self._current_option = "harina 1" if self._clave == "harina_para_prefermento" else "poolish"
                self.async_write_ha_state()
                
            self.async_on_remove(
                async_track_state_change_event(self.hass, ["number.harina_1", "number.harina_2", "number.harina_3", "number.prefermento"], _on_bascula_change)
            )
