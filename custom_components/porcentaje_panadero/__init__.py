...
# IMPORTACIÓN: Traemos las funciones de lectura/escritura y el ID del botón de levadura
from .button import obtener_tipo_levadura_actual, establecer_tipo_levadura_actual, ID_BOTON_LEVADURA
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def forzar_inyeccion_slider(nombre_base, valor):
        ...
        # CORRECCIÓN DEFINITIVA: Se añade [0] para extraer el dominio como string plano
        ...
        # CORRECCIÓN DEFINITIVA: Se añade [0] en el bucle secundario de variantes
        for entidad_id in variantes:
    ...
    async def confirmar_sobreescritura_service_dos(call: ServiceCall):
    ...
    async def cargar_formula_en_sliders_service(call: ServiceCall):
        ...
        if nombre_id in data:
            ...
            try:
                for entry_id, platform in hass.data["button"]._platforms.items():
                    if platform.domain == "button":
                        for entidad in platform.entities.values():
                            if hasattr(entidad, "unique_id") and entidad.unique_id == "porcentaje_panadero_pan_alternar_tipo_levadura_unique":
